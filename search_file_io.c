...
int get_content_from_file(struct node *ptr){
    ...
    while (fgets(content,sizeof(content),input_file)!=NULL) {
...
int write_content_on_file(struct node *ptr){
    FILE *output_file;
...
void search(struct node *list,int time){
    while(list!=NULL){
        if(list->strt_time==time){
            printf("%s at %d",list->activity,time);
            break;
        }else{
            list=list->next;
        }
