...
struct event_date *event_date_insert(struct event_date *event_date_list, int start_month, int end_month, int start_date, int end_date){
    ...
    struct event_date *event_date_last = event_date_list; //used to locate the last node in linked list
    ...
    while (event_date_last -> next != NULL)
        event_date_last = event_date_last -> next;
 
    /* insert the last node */
    event_date_last -> next = new_event_date;
    return event_date_list;
}
...
struct event_content *event_content_insert(struct event_content *event_content_list, char *name, int start_time, int end_time, int remainder){
    ...
    struct event_content *event_date_last = event_content_list; //used to locate the last node in linked list
    ...
    /* locate the last node */
    while (event_content_list -> next != NULL)
        event_content_list = event_content_list -> next;
 
    /* insert the last node */
    event_content_list -> next = new_event_content;
    return event_content_list;
}
...
int main(int argc, const char * argv[]) {
    ...
    first1 = event_date_insert(first1, 10, 10, 9, 20);
    print_list1(first1);
